...
//! This matches the TypeScript plugin's protocol in `PeerManager.ts`.
...
///
/// Sent as binary WebSocket frame containing UTF-8 JSON. The TypeScript
/// equivalent is in `plugin/src/network/PeerManager.ts` (sendHandshake method).
///
/// # Wire Format
///
/// ```json
...
mod tests {
    ...
    #[test]
    fn test_handshake_equality() {
        let msg1 = HandshakeMessage::new("peer-1", "client");
        let msg2 = HandshakeMessage::new("peer-1", "client");
        let msg3 = HandshakeMessage::new("peer-2", "client");
    ...
    fn test_wire_format_compatibility() {
        // This is the exact format TypeScript produces
        let ts_wire = br#"{"type":"handshake","peerId":"test-client-id","role":"client"}"#;
        let parsed = HandshakeMessage::from_binary(ts_wire).unwrap();
